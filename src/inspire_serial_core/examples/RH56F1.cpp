...
void deviceControlThread(const std::string& deviceName, const std::string& port, std::shared_ptr<SerialPortBase> device,
    ...
    while (g_running.load()) {
        try {
            ...
            // 1. 写入角度设定值（angleSet）
            // std::vector<int> values = {4000, 4000, 4000, 4000, 4000, 4000};
            // bool speedSuccess = protocol->writeRegister(device, "defaultSpeedSet", values);

            IoError writeSuccess = protocol->writeRegister(device, "angleSet", angles);
            (void)writeSuccess;

            // 2. 读取当前角度值（angleAct）
            auto readResult = protocol->readRegister(device, ringBuffer, "angleAct", 12);
            (void)readResult;
...
int main() {
    try {
        ...
        for (const auto& [port, deviceInfo] : deviceConfig) {
            ...
            // 为每个设备创建独立协议对象
            std::shared_ptr<Protocol> protocol =
                ConfigLoader::createProtocolFromConfig("config/device_protocol_config.yaml");
