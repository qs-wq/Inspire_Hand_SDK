...
std::vector<uint8_t> RH56H1_canfd_Protocol::buildWriteCommand(int address, const std::vector<int>& values) {
...
IoError RH56H1_canfd_Protocol::writeRegister(Device device, const std::string& reg_name,
...
RegisterReadResult RH56H1_canfd_Protocol::readRegister(Device device, RingBuffer& ringBuffer,
...
TouchReadResult RH56H1_canfd_Protocol::readTouchData(Device device, RingBuffer& ringBuffer, int version) {
    ...
    try {
        ...
        // 下面的解析逻辑参考 RH56H1_485_Protocol::parseTouchData(version=1)
...
// 自动注册 RH56H1 CAN-FD 协议
// 在设备配置文件 device_protocol_config.yaml 中，可通过：
//   protocol:
//     type: RH56H1_canfd
// 来选择使用本协议。
REGISTER_PROTOCOL("RH56H1_canfd", RH56H1_canfd_Protocol);
