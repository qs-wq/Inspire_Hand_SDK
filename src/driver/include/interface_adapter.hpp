...
public:
    InterfaceAdapter(IRegisterIoBackend& backend, const DeviceNodeConfig& config, RosEntityMaps maps)
        : backend_(backend), config_(config), maps_(maps) {}
    ...
    virtual void publishRegisterData(const TopicConfig& topic_config, const std::vector<int>& values) = 0;

    virtual void publishTouchData(const TopicConfig& topic_config, const TouchDataResult& touchData, int version) = 0;
...
protected:
    ...
    typename rclcpp::Service<ServiceT>::SharedPtr makeGroupedService(const std::string& name, CallbackT&& callback) {
        return backend_.ioNode()->template create_service<ServiceT>(name, std::forward<CallbackT>(callback),
    ...
    makeGroupedSubscription(const std::string& name, const rclcpp::QoS& qos, CallbackT&& callback) {
        rclcpp::SubscriptionOptions options;
        options.callback_group = backend_.ioServiceCallbackGroup();
        return backend_.ioNode()->template create_subscription<MessageT>(name, qos, std::forward<CallbackT>(callback),
                                                                         options);
    }
...
std::unique_ptr<InterfaceAdapter> makeInterfaceAdapter(const std::string& interfaces_profile,
                                                       IRegisterIoBackend& backend, const DeviceNodeConfig& config,
                                                       RosEntityMaps maps);
