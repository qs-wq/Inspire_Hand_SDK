...
void RegisterController::applyInitialRegisters() {
    ...
    for (const auto& initial : config_.initial_registers) {
        const IoError err = writeRegister(initial.register_name, initial.values);
        if (isOk(err)) {
            logger->info("[{}] 启动初始写入 {} 成功 ({} 个值)",
                get_name(), initial.register_name, initial.values.size());
        } else {
            logger->warn("[{}] 启动初始写入 {} 失败: {}",
                get_name(), initial.register_name, toString(err));
        }
    }
}
